...
public class BattleTurnP : MonoBehaviour
{
    CardManagerP manager;
    ...
    private bool CaE_bufferf;
    private int[] HE_buffer = new int[2];
    private bool HE_bufferf;
    private PhotonView photonview;
    private object[] o_buffer = new object[10];
    private bool o_bufferf;
    private bool RPCsynch = false;
    ...
    void Start()
    {
        ...
        manager = GetComponent<CardManagerP>();
        for(int i = 0; i < 23; i++)
        {
    ...
    IEnumerator Battle()
    {
        ...
        if (manager.level == Level.hard ||manager.level==Level.expert)
        {
            ...
            yield return new WaitUntil(() => HE_bufferf);
            HE_bufferf = false;
            SynchCheckB();
            yield return new WaitUntil(() => RPCsynch);
            for (int i = 0; i < 2; i++)
        ...
        yield return new WaitUntil(() => CaE_bufferf);
        CaE_bufferf = false;
        SynchCheckB();
        yield return new WaitUntil(() => RPCsynch);
        for(int i = 0; i < 4; i++)
    ...
    IEnumerator FoolE(string type)
    {
        ...
        else
        {
            if (selected)
            {
                if (selectName[0] == "0") fool_obj = player.Field[0].transform.GetChild(0);
                else if (selectName[0] == "1") fool_obj = player.Field[1].transform.GetChild(0);
                else
                {
                    GameObject obj = new GameObject(selectName[0]);
                    obj.tag = enemy.tag;
                    obj.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
                    obj.AddComponent<SpriteRenderer>().sprite = manager.cardback;
                    obj.transform.position = player.Trash.transform.position;
                    player.Trash.Salvage(selectName[0]);
                    fool_obj = obj.transform;
                }
                ...
                yield return new WaitUntil(() => RPCsynch);
                yield return new WaitUntil(() => selected);
                enemy.Hand2Trash(selectName[0]);
                yield return new WaitUntil(() => enemy.synch);
                enemy.Handset(fool_obj);
                yield return new WaitUntil(() => enemy.set);
                enemy.Flipura(new List<GameObject>{ fool_obj.gameObject});
                yield return new WaitUntil(() => enemy.flipb);
            }
            FoolFlag[1] = false;
        }
    ...
    IEnumerator TurnEnd()
    {
        ...
        if(FoolFlag[0] && player.HandCardNum() > 0 && FoolFlag[1] && enemy.HandCardNum() > 0)
        {
            ...
            if (PhotonNetwork.isMasterClient)
            {
                ...
                o_buffer[0] = rand;
                buffersend("EffectOrder");
            }
            yield return new WaitUntil(() => o_bufferf);
            SynchCheckB();
            yield return new WaitUntil(() => RPCsynch);
            if ((int)o_buffer[0]== 0)
            {
    ...
    //photon
    //ACK制御
    public void SynchCheckB() { StartCoroutine("synchCheckB"); }
    IEnumerator synchCheckB()
    {
        RPCsynch = false;
        if (PhotonNetwork.isMasterClient)
        {
            yield return new WaitUntil(() => ackB);
            ackB = false;
            photonview.RPC("RPCackB", PhotonTargets.Others);
        }
        else
        {
            photonview.RPC("RPCackB", PhotonTargets.Others);
    ...
    [PunRPC]
    public void RPCackB() { ackB = true; }
    private bool ackB { get; set; }
    //データの送信
    public void buffersend(string type)
    {
        string data = "";
        //カード効果
        if (type == "CaE")
        {
            for (int i = 0; i < CaE_buffer.Length; i++)
            {
                if (CaE_buffer[i] == -1) data = data + CaE_buffer[i].ToString() + ",";
                else data = data + ((CaE_buffer[i] + 2) % 4).ToString() + ",";
            }
            photonview.RPC("RPCdataReceiveB", PhotonTargets.Others, data, type);
            CaE_bufferf = true;
        }
        //役
        else if (type == "HE")
        {
            for (int i = 0; i < HE_buffer.Length; i++)
            {
                if (HE_buffer[i] == -1) data = data + HE_buffer[i].ToString() + ",";
                else data = data + ((HE_buffer[i] + 1) % 2).ToString() + ",";
            }
            photonview.RPC("RPCdataReceiveB", PhotonTargets.Others, data, type);
            HE_bufferf = true;
        }
        ...
        {
            for (int i = 0; i < 4; i++) data = data + o_buffer[(i + 2) % 4].ToString() + ",";
            Debug.Log(data);
            photonview.RPC("RPCdataReceiveB", PhotonTargets.Others, data, type);
            o_bufferf = true;
        }
        //止水とチャンスのランダム選択
        else if (type == "HE41_149")
        {
            for (int i = 0; i < player.Hand.Length; i++) data = data + o_buffer[i].ToString() + ",";
            Debug.Log(data);
            photonview.RPC("RPCdataReceiveB", PhotonTargets.Others, data, type);
            o_bufferf = true;
        }
        //効果の順番極め
        else if (type == "EffectOrder")
        {
            data = data + (((int)o_buffer[0]+1)%2).ToString() + ",";
            Debug.Log(data);
            photonview.RPC("RPCdataReceiveB", PhotonTargets.Others, data, type);
            o_bufferf = true;
        }
    ...
    //データの受信
    [PunRPC]
    public void RPCdataReceiveB(string data,string type)
    {
        string[] spdata = data.Split(',');
        if (type=="CaE")
        {
            for (int i = 0; i < spdata.Length - 1; i++) CaE_buffer[i]=int.Parse(spdata[i]);
            CaE_bufferf = true;
        }
        else if (type == "HE")
        {
            for(int i = 0; i < spdata.Length - 1; i++) HE_buffer[i] = int.Parse(spdata[i]);
            HE_bufferf = true;
        }
        else if (type == "HE1816")
        {
            for (int i = 0; i <4; i++) o_buffer[i] = spdata[i];
            o_bufferf = true;
        }
        else if (type == "HE41_149")
        {
            for (int i = 0; i < player.Hand.Length; i++) o_buffer[i] = int.Parse(spdata[i]);
            o_bufferf = true;
        }
        else if (type == "EffectOrder")
        {
            o_buffer[0] = int.Parse(spdata[0]);
            o_bufferf = true;
        }
    }
    private void bufferReset()
    {
        for (int i = 0; i < o_buffer.Length; i++) o_buffer[i] = null;
        o_bufferf = false;
    }
    ...
    IEnumerator HE41(string tag)
    {
        ...
        if (tag.Contains("Player"))
        {
            ...
            if (PhotonNetwork.isMasterClient)
            {
                ...
                for (int i = 0; i < random.Length; i++) o_buffer[i] = random[i];
                buffersend("HE41_149");
            }
            yield return new WaitUntil(() => o_bufferf);
            SynchCheckB();
            yield return new WaitUntil(() => RPCsynch);
            int counter = 0;
            //bufferの最後まではデータはないがそこまで行かずにbreakする
            for (int i = 0; i < o_buffer.Length; i++)
            {
                if (enemy.Hand[(int)o_buffer[i]].childCount != 0)
                {
                    enemy.Fieldset(enemy.Hand[(int)o_buffer[i]].GetChild(0));
                    yield return new WaitUntil(() => enemy.set);
                    if (++counter == 2) break;
                }
            }
        ...
        else if (tag.Contains("Enemy"))
        {
            ...
            for (int i = 0; i < o_buffer.Length; i++)
            {
                if (player.Hand[(int)o_buffer[i]].childCount != 0)
                {
                    player.Fieldset(player.Hand[(int)o_buffer[i]].GetChild(0));
                    yield return new WaitUntil(() => player.set);
                    if (++counter == 2) break;
                }
            }
    ...
    IEnumerator HE1816(string tag)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            pobj.name = (string)o_buffer[i];
            eobj.name = (string)o_buffer[i + 2];
            pobj.tag = player.tag;
            eobj.tag = enemy.tag;
            pobj.GetComponent<SpriteRenderer>().sprite = manager.arcana[arcana2num(pobj.name)];
            eobj.GetComponent<SpriteRenderer>().sprite = manager.arcana[arcana2num(eobj.name)];
            ...
            player.Trash.Salvage(pobj.name);
            enemy.Trash.Salvage(eobj.name);
    ...
    IEnumerator HE106(string tag)
    {
        ...
        else if (tag.Contains("Enemy"))
        {
            ...
            yield return new WaitUntil(() => selected);
            manager.state = State.Battle;
            Transform slc = player.Field[int.Parse(selectName[0])].GetChild(0);
            slc.GetComponent<SpriteRenderer>().sprite = manager.arcana[6];
            slc.name = "The Lovers";
        }
        HEtable[10, 6] = false;
    ...
    IEnumerator HE186_2(string tag)
    {
        ...
        else if (tag.Contains("Enemy"))
        {
            if (player.HandCardNum() > 0)
            {
                ...
                yield return new WaitUntil(() => RPCsynch);
                yield return new WaitUntil(() => selected);
                player.Fieldset(player.Hand[int.Parse(selectName[0])].GetChild(0));
                yield return new WaitUntil(() => player.set);
            }
        ...
        if (tag.Contains("Player"))
        {
            ...
            if (enemy.HandCardNum() > 0)
            {
                ...
                for (int i = 0; i < o_buffer.Length; i++)
                {
                    if (enemy.Hand[(int)o_buffer[i]].childCount != 0)
                    {
                        enemy.Flip(new List<GameObject> { enemy.Hand[(int)o_buffer[i]].GetChild(0).gameObject });
                        yield return new WaitUntil(() => enemy.flipb);
                        enemy.Hand2Trash(((int)o_buffer[i]).ToString());
                        yield return new WaitUntil(() => enemy.synch);
                        if (++counter == 2) break;
                    }
                }
        ...
        else if (tag.Contains("Enemy"))
        {
            ...
            if (player.HandCardNum() > 0)
            {
                ...
                for (int i = 0; i < o_buffer.Length; i++)
                {
                    if (player.Hand[(int)o_buffer[i]].childCount != 0)
                    {
                        player.Hand2Trash(((int)o_buffer[i]).ToString());
                        yield return new WaitUntil(() => player.synch);
                        if (++counter == 2) break;
                    }
                }
                ...
                for (int i = 0; i < selectCount; i++)
                {
                    player.DT2Hand(selectName[i], "Trash");
                    yield return new WaitUntil(() => player.synch);
                }
            }
        }
    ...
    IEnumerator HE205(string tag)
    {
        ...
        else if (tag.Contains("Enemy"))
        {
            ...
            int num1 = cardlist[0].Number < cardlist[1].Number ? cardlist[0].Number : cardlist[1].Number;
            int num2 = cardlist[0].Number < cardlist[1].Number ? cardlist[1].Number : cardlist[0].Number;
            if (!(num1 == 5 && num2 == 20)) for (int i = 0; i < HE_buffer.Length; i++) HE_buffer[i] = -1;
        }
        hand_effect = false;
    }
    ...
    {
        Debug.Log(tag + ":" + num2arcana(i) + " Effect");
    ...
    IEnumerator Magician(string tag)
    {
        ...
        else if(tag.Contains("Enemy") && enemy.HandCardNum() > 0)
        {
            ...
            if ((bool)HEgetter(121, tag)) SelectSetting(player.HandCardNum());
            else if ((bool)HEgetter(91, tag)) SelectSetting(2);
            else SelectSetting(1);
            SynchCheckB();
            yield return new WaitUntil(() => RPCsynch);
    ...
    {
        photonview.RPC("RPCselectedAction", PhotonTargets.Others, str);
        if (str == "No")
        {
            selectCansel = true;
            return;
    ...
    [PunRPC]
    public void RPCselectedAction(string str)
    {
        if (str == "No")
        {
            selectCansel = true;
