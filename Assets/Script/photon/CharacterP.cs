...
public abstract class CharacterP : MonoBehaviour
{
    ...
    public TrashzoneP Trash;
    public Transform Deckzone;
    public CardManagerP manager;
    ...
    public BattleTurnP battle;
    public int Decktop { get; set; }
    public int FieldCount { get; set; }
    public int HP { get; set; }
    public bool AD { get; set; }
    
    public bool[] HEbuf { get; set; }
    ...
    protected int[] i_buffer = new int[25];
    protected bool i_bufferf;
    protected bool RPCsynch = false;
    ...
    private void Awake()
    {
        ...
        {
            GameObject zone = Instantiate((GameObject)Resources.Load("Prefab/photon/HandZone"));
            zone.tag = tag;
            ...
            zone.GetComponent<zoneScriptP>().number = i.ToString();
            Hand[i] = zone.transform;
        }
        for (int i = 0; i < fieldmax; i++)
        {
            GameObject zone = Instantiate((GameObject)Resources.Load("Prefab/photon/FieldZone"));
            zone.tag = tag;
            ...
            foreach (Transform child in zone.transform)
            {
                if (child.GetComponent<BoxCollider2D>() != null)
                {
                    ...
                    child.GetComponent<zoneScriptP>().number = i.ToString();
                    Field[i] = child;
                }
            }
    ...
    {
        for (int i = 0; i < i_buffer.Length; i++) i_buffer[i] = -1;
        i_bufferf = false;
    }
    public void bufferSet(int[] array)
    {
        for (int i = 0; i < array.Length; i++) i_buffer[i] = array[i];
        i_bufferf = true;
    }
    //カード操作
    /*デッキをshuffle
    ...
    IEnumerator deckShuffle()
    {
        ...
        {
            manager.datasynch("deck",tag);
        }
        yield return new WaitUntil(() => i_bufferf);
        for (int i = 0; i < Deck.Count; i++) Deck[i] = i_buffer[i];
    ...
    IEnumerator handShuffle()
    {
        ...
        if (PhotonNetwork.isMasterClient)
        {
            manager.datasynch("hand",tag);
        }
        yield return new WaitUntil(() => i_bufferf);
        for (int i = 0; i < 4; i++) after[i] = i_buffer[i];
     ...
     * 同期はsynch
     */
    public void Draw(int num) { StartCoroutine(Draws(num)); }
    IEnumerator Draws(int num)
    {
        AnimationClip clip = new AnimationClip();
        ...
        {
            locked = false;
            for (int i = 0; i < Field.Length; i++) Field[i].GetComponent<zoneScriptP>().Locked = false;
            for (int i = 0; i < Hand.Length; i++) Hand[i].GetComponent<zoneScriptP>().Locked = false;
        }
    ...
    IEnumerator handSet(Transform card)
    {
        ...
        if (GetHEbuf(2221) || GetHEbuf(186) || (satoriF - manager.GetBattleTurn()) > 0)
        {
    ...
    {
        manager.CharacterMediator("Select", tag, str);
    }
    //managerから呼び出される
    public void rPCselectedAction(string str)
    {
    ...
    //ACK同期
    public void SynchCheck()　{　StartCoroutine("synchCheck");　}
    IEnumerator synchCheck()
    {
        RPCsynch = false;
        if (PhotonNetwork.isMasterClient)
        {
            ...
            ack = false;
            photonview.RPC("RPCack", PhotonTargets.Others);
        }
        else
        {
            photonview.RPC("RPCack", PhotonTargets.Others);
    ...
    public void RPCack()
    {
        manager.CharacterMediator("Ack", tag, "");
    }
    //managerがtrueにする
